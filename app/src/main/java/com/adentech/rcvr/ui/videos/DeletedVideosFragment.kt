...
class DeletedVideosFragment : BaseFragment<ScanViewModel, FragmentDeletedVideosBinding>() {
    ...
    private fun adjustVideos(videosLists: ArrayList<FileModel>) {
        val adjustedVideoList: ArrayList<FileModel> = ArrayList()
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            adjustedVideoList.addAll(videosLists)
        } else {
            adjustedVideoList.addAll(ImagesFilesCollector.foundVideoList)
        }
        val min = adjustedVideoList.size / 5
        val max = adjustedVideoList.size / 2
        val count = if (adjustedVideoList.size / 50 > 3) {
            160
        } else {
            adjustedVideoList.size / 10 + 1
        }
        ...
        lifecycleScope.launch {
            if (adjustedVideoList.isNotEmpty()) {
                ...
                if (adjustedVideoList.isNotEmpty()) {
                    for (img in 1..indexesArray.size) {
                        ...
                        withContext(Dispatchers.Main) {
                            val chosenVideo: FileModel =
                                adjustedVideoList[indexesArray[img - 1]]
